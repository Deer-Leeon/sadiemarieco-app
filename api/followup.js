 ...
 * POST /api/followup
 *
 * Receives Cal.com workflow webhook events configured to fire 24h after an
 * appointment ends and sends a "thank you / come back" SMS via Twilio.
 * Always returns 200 OK so Cal.com's workflow engine won't time out or
 * retry. Errors are logged for our own debugging.
 *
...
const SITE_URL = 'https://sadiemarieco.vercel.app';
...
const buildMessage = ({ clientName }) =>
  `Hi ${clientName}! 🤍 Thank you so much for visiting Sadie Marie yesterday. We loved having you in the studio! When you are ready, you can book your next session here: ${SITE_URL}`;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.error('[api/followup] invalid JSON body:', err);
  ...
  // The followup message doesn't include the management link, but we still
  // grab uid so logs can be cross-referenced with bookings.
  const bookingUid = unwrap(payload.uid);

  if (!clientPhone) {
    console.warn('[api/followup] no phone number on payload — skipping SMS', { bookingUid });
  ...
  if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_PHONE_NUMBER) {
    console.error('[api/followup] Twilio env vars missing — cannot send SMS', {
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      to: clientPhone,
      body: buildMessage({ clientName })
    });
    console.log('[api/followup] SMS sent', { sid: message.sid, to: maskPhone(clientPhone), bookingUid });
    return res.status(200).json({ ok: true, smsSid: message.sid });
  } catch (err) {
    console.error('[api/followup] Twilio send failed:', {
      bookingUid,
