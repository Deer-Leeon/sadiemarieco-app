-- Lifetime Late-Change + No-show breakdown counters on clients.
-- Totals: no_show_count (existing), late_change_count (new).
-- Sub-counts never decrease; writers keep totals = sum of parts.
...
COMMENT ON COLUMN clients.late_change_count IS
  'Lifetime 2h–24h cancel/reschedule fee events. Never decrements. Equals cancel + reschedule sub-counts.';
...
-- Historical no-shows were admin-driven; attribute existing total to admin.
UPDATE clients
SET no_show_admin_count = no_show_count
WHERE no_show_admin_count = 0
  AND no_show_count > 0;

-- Backfill late-change cancels from charged late-cancel appointment rows.
UPDATE clients c
SET
