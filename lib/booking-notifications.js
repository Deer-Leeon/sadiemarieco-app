...
/** Tally form slug from the share URL: https://tally.so/r/<this-id> */
const TALLY_CONSENT_FORM_ID = process.env.TALLY_CONSENT_FORM_ID || '';
...
function buildTallyConsentLink(clientId) {
  if (!TALLY_CONSENT_FORM_ID || !clientId) return null;
  const id = String(clientId).trim();
  if (!id) return null;
  return `https://tally.so/r/${TALLY_CONSENT_FORM_ID}?client_id=${encodeURIComponent(id)}`;
}
...
function buildConfirmationMessage({
  ...
  if (!hasConsented && clientId) {
    const tallyLink = buildTallyConsentLink(clientId);
    if (tallyLink) {
      body += `\n\nPlease complete your intake form before your appointment: ${tallyLink}`;
    } else if (!TALLY_CONSENT_FORM_ID) {
      console.warn(
        '[booking-notifications] TALLY_CONSENT_FORM_ID not set — skipping intake link in SMS',
        { clientId }
      );
    }
  }
