...
async function notifyBookingConfirmed({
  ...
  // Email has its own idempotency key (`{uid}:email`); always attempt so admin
  // manual-booking complete can send even when the Cal webhook already texted.
  ...
  if (skipIfAlreadySent && (await wasBookingNotificationSent(bookingUid))) {
    return { ok: true, skipped: 'already_notified', email: emailResult };
  }
