...
async function scheduleReminderAndFeedback(
  ...
  // scheduled for. /api/remind and /api/qstash/review-request compare it
  // against the current appointments.booking_time and self-skip on mismatch.
  ...
  async function publishRemindJob(kind, notBeforeSec) {
    const claimKey = qstashSmsClaimKey(bookingUid, 'lead');
    const claimed = await tryClaimWebhookEvent(claimKey);
    if (!claimed) {
  ...
  } else if (msUntilAppt > 0) {
    // Already inside the 24h/48h window (including the last 90 minutes) —
    // send the lead prep text now so a last-minute book or reschedule
    // still gets arrival instructions.
