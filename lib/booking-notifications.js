...
async function notifyAdminAppointmentStatusSms({
  ...
  if (kind === 'admin_cancel') {
    body = buildAdminCancelSms({ serviceName, bookingTime });
  } else if (kind === 'no_show_charged') {
    body = buildNoShowChargedSms({
      serviceName,
      bookingTime,
      amountCents,
    });
  } else if (kind === 'no_show') {
    body = buildNoShowNoChargeSms({ serviceName, bookingTime });
  } else {
