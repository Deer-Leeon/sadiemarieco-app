...
const { isSmsOptInTruthy } = require('./sms-reminder-guards');
const {
...
/**
 * Re-publish missing reminder jobs for upcoming confirmed, SMS-opted-in
 * appointments. Safe to run every 15 minutes — QStash claims are idempotent.
 */
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (
        result &&
        result.scheduled &&
        (result.reminderLead || result.reminder1h || result.feedback)
      ) {
        scheduled += 1;
      }
    } catch (err) {
...
async function notifyBookingConfirmed({
  ...
  let adminPush = null;
  try {
    const { notifyAdminBookingConfirmed } = require('./admin-booking-push');
    adminPush = await notifyAdminBookingConfirmed({
      bookingUid,
      bookingTime,
      clientName,
      serviceName,
