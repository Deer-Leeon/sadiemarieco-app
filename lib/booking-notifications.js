...
async function fulfillReviewRequestForBooking({ bookingUid, expectedBookingTime }) {
  ...
  const fireAtMs = reviewRequestFireAtMs(
    appointment.end_time,
    appointment.booking_time
  );
  if (Number.isFinite(fireAtMs) && Date.now() + 5000 < fireAtMs) {
    return { ok: true, skipped: 'visit_not_ended' };
  }
...
async function sendVisitUpdateSms(visit, { bookingUid = null } = {}) {
  ...
  // Prep belongs on the reminder. Repeat it here only when that reminder
  // already went — rebuildVisitJobs will not send another lead.
  vars.visitPrep = leadSent
    ? visit.visitPrep || ''
    : "You'll receive a reminder with pre-arrival instructions before your visit.";
...
async function syncVisitNotificationsAfterChange(
  bookingUid,
  { skipSingleIfIsThisBooking = false, skipVisitSms = false } = {}
) {
  ...
  for (const uid of uids) {
    ...
    if (!skipVisitSms && !thisFpNotified && (leadSent || priorNotice)) {
      const isThisSingle =
...
async function notifyAppointmentRescheduled({
  ...
  let skipStandaloneRescheduleSms = false;
  let visit = null;
  if (sendClientSms !== false) {
    try {
      ...
      if (visit && visit.services.length > 1) {
        const leadSent = await visitSms.wasVisitLeadSent(visit);
        const multiNotice = await visitSms.wasMultiVisitNotified(visit);
        // One text for the day: the visit update already lists every service
        // and the new arrival time. A second "rescheduled" SMS is redundant.
        if (leadSent || multiNotice) {
          skipStandaloneRescheduleSms = true;
        }
      }
    } catch (err) {
      console.warn(
        '[booking-notifications] visit lookup for reschedule SMS failed',
        {
          bookingUid,
          error: err instanceof Error ? err.message : String(err),
        }
  ...
  if (claimed) {
    ...
    } else if (skipStandaloneRescheduleSms) {
      sms = { ok: true, skipped: 'visit_update_covers_reschedule' };
    } else {
      const manageUrl = `${MANAGE_LINK_BASE}?uid=${encodeURIComponent(bookingUid)}`;
  ...
  if (scheduleSmsReminders && isSmsOptInTruthy(smsOptIn) && bookingTime) {
    ...
    try {
      await syncVisitNotificationsAfterChange(bookingUid, {
        ...
        skipVisitSms: sendClientSms === false,
      });
    } catch (syncErr) {
