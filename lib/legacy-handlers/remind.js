 ...
 *   • kind: '1h'  — notBefore = appointmentTime - 1h
 *
 * Contract:
...
const buildMessage = async ({ kind, serviceName, bookingTime }) => {
  if (kind === '1h') {
    return resolveReminder1hSms({ serviceName });
  }
  if (kind === '48h') {
...
module.exports = async function handler(req, res) {
  ...
  const sendClaimKey = `${bookingUid}:sms_sent:${kind === '1h' ? '1h' : 'lead'}`;
  const qstashClaimKey = `${bookingUid}:qstash_sms:${kind === '1h' ? '1h' : 'lead'}`;
