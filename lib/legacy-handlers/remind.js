...
module.exports = async function handler(req, res) {
  ...
  if (expectedBookingTime && appointment.booking_time) {
    const expectedMs = Date.parse(expectedBookingTime);
    const actualMs =
      appointment.booking_time instanceof Date
        ? appointment.booking_time.getTime()
        : Date.parse(String(appointment.booking_time));
    if (
      ...
      console.log('[api/remind] booking time changed — skipping stale job', {
        ...
        kind,
        expectedBookingTime,
        actualBookingTime: new Date(actualMs).toISOString(),
      });
      return res
        .status(200)
  ...
  // A2P: only clients who explicitly opted in should receive reminder SMS.
  if (appointment.sms_opt_in !== true) {
