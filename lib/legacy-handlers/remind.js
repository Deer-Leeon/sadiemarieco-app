 ...
 *   • kind: '24h' — notBefore = appointmentTime - 24h
...
const buildMessage = ({ kind, serviceName, bookingTime, bookingUid }) => {
  if (kind === '1h') {
    return buildReminder1hSms({ serviceName });
  }
  return buildReminder24hSms({ serviceName, bookingTime });
};
...
module.exports = async function handler(req, res) {
  ...
  const kind = body && body.kind === '1h' ? '1h' : '24h';
