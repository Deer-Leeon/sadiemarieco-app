...
const { listUpcomingClientAppointments } = require('../same-day-visit');
...
async function loadUpcomingAppointments(uid) {
  try {
    const upcoming = await listUpcomingClientAppointments(uid);
    return Array.isArray(upcoming) ? upcoming : [];
  } catch (err) {
    console.warn('[api/booking] upcoming list failed (non-fatal)', {
      uid,
      error: err && err.message,
    });
    return [];
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [changeFee, extras, upcoming] = await Promise.all([
      loadChangeFeePreview(booking.uid, start),
      loadAppointmentExtras(booking.uid),
      loadUpcomingAppointments(booking.uid),
    ]);
