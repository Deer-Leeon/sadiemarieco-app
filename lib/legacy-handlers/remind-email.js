 ...
 * 24h lashes). Scheduled by booking-notifications at confirmation time
 * with notBefore set to the target send window.
 *
 * Payload: { bookingUid, expectedBookingTime, timing: 'lead' }
 * Retired `timing: '1h'` jobs return skipped so they do not send.
...
module.exports = async function handler(req, res) {
  ...
  if (timing === '1h') {
    console.log('[api/remind-email] 1h reminder retired — skipping', {
      bookingUid,
    });
  ...
  try {
    ...
    if (!result.ok && result.error) {
      // Real delivery failure (not a skip): the idempotency claim was
      // released, so a 500 lets QStash's bounded retries re-attempt.
    ...
    // 500 → QStash retries (bounded); the send claim is released on failure.
