...
module.exports = async function handler(req, res) {
  ...
  // Block cancel once the appointment has started (or is in the past).
  try {
    let startIso = null;
    try {
      const { rows } = await sql`
        ...
        LIMIT 1
      `;
      if (rows[0] && rows[0].booking_time) {
        startIso =
          rows[0].booking_time instanceof Date
            ? rows[0].booking_time.toISOString()
            : String(rows[0].booking_time);
      }
    } catch (dbLookupErr) {
      console.warn('[api/cancel-booking] booking_time lookup failed; falling back to Cal', {
        uid,
        error: dbLookupErr && dbLookupErr.message,
      });
    }

    if (!startIso) {
      const calGet = await fetch(
        `${CAL_API_BASE}/bookings/${encodeURIComponent(uid)}`,
        {
          method: 'GET',
          headers: {
      ...
      const calPayload = await calGet.json().catch(() => null);
      const calBooking = (calPayload && calPayload.data) || calPayload;
      if (calGet.ok && calBooking && calBooking.start) {
        startIso = String(calBooking.start);
      }
    }
  ...
  try {
    ...
    // Retried once on failure: a transient Neon blip here would otherwise
    // leave the row 'confirmed' and let a 24h reminder fire for a booking
    // the client just cancelled. (Cal's BOOKING_CANCELLED webhook also
    // flips this status independently, so drift needs BOTH to fail.)
    const markCanceled = async () => {
      ...
      `;
      return { updated: updatedRows.length, existing };
    };
    try {
      let result;
      try {
        result = await markCanceled();
      } catch (firstErr) {
        console.warn('[api/cancel-booking] DB status update failed — retrying once:', {
          uid,
      ...
      if (result.updated === 0) {
        ...
        const prevStatus = String(result.existing?.status || '').toLowerCase();
        if (prevStatus === 'confirmed') {
          ...
          try {
            await notifyAdminAppointmentPush({
              ...
              requestHost: req.headers.host || req.headers['x-forwarded-host'] || null,
            });
          } catch (pushErr) {
            console.warn('[api/cancel-booking] admin iOS push failed (non-fatal)', {
              uid,
              error: pushErr && pushErr.message,
            });
          }
        }
      }
    } catch (dbErr) {
      console.error('[api/cancel-booking] DB status update failed after retry (Cal cancel succeeded):', {
        uid,
