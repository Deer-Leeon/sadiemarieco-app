...
// Must stay in sync with `CAL_ABANDON_CANCEL_REASON` / legacy list in
// `lib/booking-hold.ts`. Cal echoes this string on BOOKING_CANCELLED
// after our abandoned-checkout release (QStash delayed hold).
const SYSTEM_ABANDON_CANCEL_REASONS = new Set([
...
const isSystemAbandonCancellation = (reason) => {
  ...
  return SYSTEM_ABANDON_CANCEL_REASONS.has(trimmed);
};
...
module.exports = async function handler(req, res) {
  ...
  // Website bookings: only text when the optional sms-consent checkbox is
  // explicitly checked. Unchecked / missing = no SMS (A2P forced-consent rule).
  const smsOptIn =
