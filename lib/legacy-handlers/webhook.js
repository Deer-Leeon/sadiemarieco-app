...
module.exports = async function handler(req, res) {
  ...
  const normPhone = normaliseClientPhoneForStorage(clientPhone);
  const appointmentPhone = normPhone || null;
  let phoneTaken = false;
  if (normPhone) {
    for (const variant of clientPhoneLookupVariants(normPhone)) {
      const { rows: phoneRows } = await sql`
        SELECT 1 FROM clients WHERE phone = ${variant} LIMIT 1
      `;
      if (phoneRows[0]) {
        phoneTaken = true;
        break;
