...
} = require('../catalogue-service-title.js');
...
async function withCatalogueServiceName(rawName, calEventTypeId) {
  const fallback =
    rawName == null || String(rawName).trim() === ''
      ? 'appointment'
      : String(rawName);
  if (!calEventTypeId) return fallback;
  try {
    const { rows } = await sql`
      SELECT title
      FROM site_services
      WHERE cal_event_id = ${calEventTypeId}
        AND is_active = TRUE
      LIMIT 1
    `;
    const title = rows[0] && rows[0].title ? String(rows[0].title).trim() : '';
    if (!title) return fallback;
    return applyCatalogueTitleToServiceName(fallback, title) || fallback;
  } catch (err) {
    console.warn('[api/webhook] catalogue title lookup failed', {
      calEventTypeId,
...
async function syncAttachedExtrasTimes(parentId, bookingTime, endTime) {
  if (!parentId) return;
  try {
    await sql`
      UPDATE appointments
      SET booking_time = ${bookingTime},
          end_time = ${endTime}
      WHERE attached_to_appointment_id = ${parentId}
    `;
  } catch (err) {
    console.warn('[api/webhook] attached extras time sync failed', {
...
async function deleteUnsettledAttachedExtras(parentId) {
  if (!parentId) return;
  try {
    await sql`
      DELETE FROM appointments extra
      WHERE extra.attached_to_appointment_id = ${parentId}
        AND NOT EXISTS (
          SELECT 1
    ...
    console.warn('[api/webhook] attached extras cancel cleanup failed', {
      parentId: String(parentId),
      error: err && err.message,
    });
  }
...
function toBookingTimeIso(value, fallback) {
  if (value instanceof Date) return value.toISOString();
  if (value) return String(value);
  return fallback || null;
}

function webhookRequestHost(req) {
...
function shouldNotifyAdminOfClientCancel(status) {
  const s = String(status || '').toLowerCase();
  // /manage marks canceled_by_client before this webhook arrives; still alert.
  return (
    s === 'confirmed' ||
...
async function notifyAdminLifecyclePush(args) {
  try {
    return await notifyAdminAppointmentPush(args);
  } catch (err) {
    console.warn('[api/webhook] admin iOS push failed (non-fatal)', {
      bookingUid: args && args.bookingUid,
      kind: args && args.kind,
      error: err && err.message,
    });
...
/** BOOKING_CREATED for a moved booking — must not send confirmation SMS. */
function isRescheduleCreationPayload(payload) {
  if (!payload || typeof payload !== 'object') return false;
  if (unwrap(payload.rescheduleUid)) return true;
  if (unwrap(payload.fromReschedule && payload.fromReschedule.uid)) return true;
  const metadata =
    payload.metadata && typeof payload.metadata === 'object'
      ? payload.metadata
      : {};
  return unwrap(metadata.admin_reschedule) === 'true';
}
...
module.exports = async function handler(req, res) {
  ...
  let endTime = resolvedEndTime;
  const calEventTypeId = extractCalEventTypeId(payload);
  const serviceName = await withCatalogueServiceName(
    rawServiceName,
    calEventTypeId
  );
  ...
  if (triggerEvent === 'BOOKING_RESCHEDULED') {
    ...
    try {
      ...
      const chairMins = Number(existingBefore && existingBefore.chair_duration_mins);
      if (
        bookingTime &&
        Number.isFinite(chairMins) &&
        chairMins > 0
      ) {
        const startMs = new Date(bookingTime).getTime();
        if (!Number.isNaN(startMs)) {
          endTime = new Date(startMs + chairMins * 60_000).toISOString();
        }
      }
      ...
      const { rows: updatedRows } = await sql`
        ...
        RETURNING id, cal_event_id, status, client_phone, service_name,
                  booking_time, sms_opt_in, client_first_name, client_last_name
      `;
      if (updatedRows.length > 0) {
        ...
        // Admin iOS alert first — before extras sync / reminder emails / SMS,
        // none of which may stand between the row update and the banner.
        // notifyAppointmentRescheduled below dedupes on the same claim.
        if (updatedRows[0].status === 'confirmed' && !sameSlot) {
          const movedRow = updatedRows[0];
          await notifyAdminLifecyclePush({
            kind: 'rescheduled',
            source: isAdminReschedule ? 'admin' : 'client',
            bookingUid,
            bookingTime: toBookingTimeIso(movedRow.booking_time, bookingTime),
            clientName: clientDisplayName(movedRow),
            serviceName: movedRow.service_name || serviceName,
            appointmentId: movedRow.id ? String(movedRow.id) : null,
            requestHost: webhookRequestHost(req),
          });
        }
              ...
              const bt =
                row.booking_time instanceof Date
                  ...
                  : row.booking_time
                    ...
                    : bookingTime;
              let sendClientSms = true;
              try {
                if (await hasSkipClientSms(oldUid)) {
                  sendClientSms = false;
                }
              } catch (skipErr) {
                console.warn(
                  '[api/webhook] BOOKING_RESCHEDULED: skip_client_sms lookup failed',
                  { oldUid, error: skipErr && skipErr.message }
                );
              }
    ...
    try {
      ...
      if (systemAbandon) {
        ...
        if (updatedRows.length === 0) {
          ...
          await deleteUnsettledAttachedExtras(existing.id);
        }
        return res.status(200).json({ ok: true, event: 'BOOKING_CANCELLED' });
      }
      ...
      if (
        ...
        // Alert first — before status writes and extras cleanup, so a DB
        // hiccup in those cannot swallow the admin banner. Keyed on the
        // pre-webhook status only, so ordering does not change who is told.
        if (
          existingStatus === 'canceled_by_admin' ||
          existingStatus === 'confirmed'
        ) {
          await notifyAdminLifecyclePush({
            kind: 'canceled',
            source: 'admin',
            bookingUid,
            bookingTime: toBookingTimeIso(existing.booking_time),
            clientName: clientDisplayName(existing),
            serviceName: existing.service_name,
            appointmentId: existing.id ? String(existing.id) : null,
            requestHost: webhookRequestHost(req),
          });
        }
      ...
      // Admin iOS alert before the Stripe penalty / status write / extras
      // cleanup: none of those may stand between Cal's cancel and the
      // banner. Reschedule-driven cancels get the reschedule alert instead.
      if (!rescheduleCancel && shouldNotifyAdminOfClientCancel(existingStatus)) {
        await notifyAdminLifecyclePush({
          kind: 'canceled',
          source: 'client',
          bookingUid,
  ...
  try {
    await sql`
      ...
      ON CONFLICT (cal_event_id) DO UPDATE SET
        ...
        end_time = CASE
          WHEN appointments.chair_duration_mins IS NOT NULL
          THEN EXCLUDED.booking_time
            + (appointments.chair_duration_mins * INTERVAL '1 minute')
  ...
  if (treatAsManualAdmin) {
    if (isRescheduleCreationPayload(payload)) {
      console.log(
        '[api/webhook] BOOKING_CREATED: reschedule — skipping confirmation SMS',
        { bookingUid }
      );
      return res.status(200).json({
        ok: true,
        dbWritten: true,
        status: 'confirmed',
        skipped: 'reschedule_not_confirmation',
      });
    }
