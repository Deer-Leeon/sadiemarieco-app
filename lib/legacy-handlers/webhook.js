 ...
 *                          reminder QStash run only after checkout confirm.
 *                          Public pending upserts also schedule a delayed
 *                          /api/qstash/release-hold so abandoned holds clear
 *                          even when /api/booking/init fails.
...
module.exports = async function handler(req, res) {
  ...
  // fails (e.g. missing email + SMS opt-out). Admin manual bookings stay
  // pending until /api/admin/manual-booking/complete — do not schedule.
  // Duplicate schedules with init/book-create are idempotent on release.
  if (!isManualAdmin && bookingUid) {
    try {
