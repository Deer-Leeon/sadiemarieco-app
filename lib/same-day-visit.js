...
function visitMultiNoticeKey(visitKeyValue) {
  return `${visitKeyValue}:multi_notice`;
}
...
function prepCopyForServiceName(serviceName) {
  ...
  if (
    ...
    services.push({ reminderKind: 'brows' });
  }
  if (lower.includes('lash')) {
    services.push({ reminderKind: 'lashes' });
  }
  return visitPrep(services);
}

async function prepCopyForAppointment(
  serviceName,
  bookingTime,
  endTime,
  { calEventTypeId = null, bookingUid = null } = {}
) {
  let typeId =
    calEventTypeId == null || calEventTypeId === ''
      ? null
      : Number(calEventTypeId);
  if (!Number.isFinite(typeId)) typeId = null;

  if ((!Number.isFinite(typeId) || !serviceName) && bookingUid) {
    const anchor = await loadAnchorAppointment(bookingUid);
    if (anchor) {
      if (!serviceName) serviceName = anchor.service_name;
      if (!bookingTime) bookingTime = isoFromSql(anchor.booking_time);
      if (!endTime) endTime = isoFromSql(anchor.end_time);
      if (anchor.cal_event_type_id != null) {
        typeId = Number(anchor.cal_event_type_id);
      }
    }
  }
  ...
  try {
    ...
    if (resolved.reminderKind) {
      return visitPrep([{ reminderKind: resolved.reminderKind }]);
    }
    const inferred = lookup.inferReminderKindFromServiceName(serviceName);
    if (inferred) return visitPrep([{ reminderKind: inferred }]);
  } catch (err) {
    console.warn('[same-day-visit] prep copy lookup failed', {
      error: err instanceof Error ? err.message : String(err),
    });
  }
  return visitPrep([]);
...
/**
 * Every upcoming confirmed Cal booking for this client (any day).
 * Same identity matching as same-day visit SMS: client_id or phone digits,
 * no checkout holds, no admin extras, not started.
 */
async function listUpcomingClientAppointments(bookingUid) {
  const anchor = await loadAnchorAppointment(bookingUid);
  if (!anchor) return [];
  const clientId = anchor.client_id ? String(anchor.client_id) : null;
  const digits = phoneDigits(anchor.client_phone);
  ...
  for (const row of rows) {
    ...
    upcoming.push({
      uid,
      service_name: formatServiceTitle(row.service_name),
      start: isoFromSql(row.booking_time),
      end: isoFromSql(row.end_time) || isoFromSql(row.booking_time),
...
async function wasMultiVisitNotified(visit) {
  if (!visit?.key) return false;
  return webhookExists(visitMultiNoticeKey(visit.key));
}

async function markMultiVisitNotified(visit) {
  if (!visit?.key) return;
  await tryClaimWebhookEvent(visitMultiNoticeKey(visit.key));
}
...
module.exports = {
  ...
  visitCopyVars,
  visitPrep,
  prepCopyForServiceName,
  prepCopyForAppointment,
  wasVisitLeadSent,
  wasVisitFingerprintNotified,
  wasMultiVisitNotified,
  markVisitFingerprintNotified,
  markMultiVisitNotified,
  markVisitLeadSent,
