...
/** @typedef {'confirmation'|'reminder_24h'|'reminder_1h'|'reschedule'|'admin_cancel'|'no_show_no_charge'|'no_show_charged'|'late_cancel_fee'|'client_cancel_early'|'client_cancel_late_no_fee'|'client_cancel_after_start'|'checkout_abandoned'|'booking_pending'|'phone_cancel'|'feedback_day_after'} SmsTemplateKey */
...
const SMS_TEMPLATE_META = Object.freeze({
  ...
  reminder_24h: {
    ...
    triggers: [
      'System sends via QStash about 24 hours before a confirmed appointment (POST /api/remind).',
    ],
  ...
  no_show_no_charge: {
    ...
    triggers: [
      'Admin marks a no-show without charging a fee (strike only).',
    ],
    allowedPlaceholders: ['service', 'date', 'time'],
    requiredPlaceholders: [],
    defaultBody:
  ...
  no_show_charged: {
    ...
    triggers: [
      'Admin marks a no-show and the 50% no-show fee charge succeeds.',
    ],
  ...
  late_cancel_fee: {
    title: 'Late-cancel fee receipt',
    triggers: [
      'Client cancels within 24 hours and the late-cancel fee charge succeeds (Cal webhook).',
    ],
    allowedPlaceholders: ['service', 'date', 'time', 'amount'],
    requiredPlaceholders: ['amount'],
    defaultBody:
      'Your {{service}} on {{date}} at {{time}} was canceled. A late-cancel fee of {{amount}} was charged to your card on file.',
    sendingLive: true,
  },
  ...
  client_cancel_after_start: {
    title: 'Client cancel after start time',
    triggers: [
      'Client cancels after the appointment start time has already passed.',
    ],
    allowedPlaceholders: ['service', 'date', 'time', 'siteUrl'],
    requiredPlaceholders: [],
    defaultBody:
      'Your {{service}} on {{date}} at {{time}} was canceled after the appointment time. Reach out if you\'d like to rebook: {{siteUrl}}.',
    sendingLive: false,
  },
  checkout_abandoned: {
  ...
  booking_pending: {
    ...
    triggers: [
      'Client has a pending hold and still needs to complete Stripe checkout to confirm.',
    ],
    allowedPlaceholders: ['service', 'date', 'time', 'manageUrl', 'siteUrl'],
    requiredPlaceholders: [],
    defaultBody:
      'Your {{service}} on {{date}} at {{time}} is held — finish checkout to confirm: {{manageUrl}}. Spots are released if payment isn\'t completed.',
    sendingLive: false,
  },
  phone_cancel: {
    title: 'Phone / client cancel (admin)',
    triggers: [
      'Admin records that the client canceled by phone (or marks canceled_by_client without the admin-cancel path).',
    ],
    allowedPlaceholders: ['service', 'date', 'time', 'siteUrl'],
    requiredPlaceholders: [],
    defaultBody:
      'Your {{service}} on {{date}} at {{time}} has been canceled as requested. Book again anytime at {{siteUrl}}.',
    sendingLive: false,
  },
  feedback_day_after: {
