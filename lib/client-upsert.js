 ...
 *
 * Never retain Cal placeholder emails (@sms.cal.com, bookings+…): when the
 * incoming email is null, clear any existing placeholder rather than
 * COALESCE-keeping it.
 */
...
/**
 * Resolve email for an UPDATE: real incoming wins; else keep existing only
 * if it is also real (clears @sms.cal.com / bookings+ junk).
 */
async function resolveEmailForClientUpdate(clientId, incomingEmail) {
  const trimmed = normalizeClientEmailForStorage(incomingEmail);
  if (trimmed) return trimmed;
  const { rows } = await sql`
    SELECT email FROM clients WHERE id = ${clientId} LIMIT 1
  `;
  return normalizeClientEmailForStorage(rows[0]?.email);
}
...
async function upsertClientByPhonePrimary({
  ...
  if (existingId) {
    const emailToStore = await resolveEmailForClientUpdate(
      existingId,
      trimmedEmail
    );
    await sql`
