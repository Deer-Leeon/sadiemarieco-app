...
function rescheduleVars({ serviceName, bookingTime, manageUrl, visitPrep }) {
  return {
    service: formatServiceTitle(serviceName),
    date: bookingTime ? formatStudioDate(bookingTime) : '',
    time: bookingTime ? formatStudioTime(bookingTime) : '',
...
function visitSmsVars(args) {
  return {
    service: args.serviceName || args.visitServices || '',
    date: args.date || '',
    time: args.arrivalTime || args.time || '',
    arrivalTime: args.arrivalTime || args.time || '',
    visitServices: args.visitServices || '',
    visitPrep: args.visitPrep || '',
    manageUrl: args.manageUrl || '',
  };
}

async function resolveVisitConfirmationSms(args) {
  return resolveSmsCopy('confirmation_visit', visitSmsVars(args));
}

async function resolveVisitReminderSms(args) {
  return resolveSmsCopy('reminder_visit', visitSmsVars(args));
}

async function resolveVisitUpdateSms(args) {
  return resolveSmsCopy('visit_update', visitSmsVars(args));
}
...
async function resolveRescheduleSms(args) {
  ...
  let body = effectiveBody('reschedule', stored);
  // Older studio overrides omit {{visitPrep}}. Keep sending the same
  // lash/brow instructions visit-update texts already include.
  if (!body.includes('{{visitPrep}}')) {
    if (body.includes("I'm so excited to see you!")) {
      body = body.replace(
        "I'm so excited to see you!",
        "{{visitPrep}} I'm so excited to see you!"
      );
    } else if (body.includes('Manage, reschedule, or cancel:')) {
      body = body.replace(
        'Manage, reschedule, or cancel:',
        '{{visitPrep}} Manage, reschedule, or cancel:'
      );
    } else {
      body = `${body} {{visitPrep}}`;
    }
  }
  return renderSmsTemplate(body, rescheduleVars(args));
}
