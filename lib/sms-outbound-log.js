...
const LABEL_ALIASES = Object.freeze({
  ...
  '24h': 'reminder_24h',
  '1h': 'reminder_1h',
  reminder_visit: 'reminder_visit',
  visit_update: 'visit_update',
  confirmation_visit: 'confirmation_visit',
});

let ensureTablePromise = null;

/** Idempotent — staging (and any other DB) may not have been migrated yet. */
async function ensureSmsOutboundLogTable() {
...
async function recordOutboundSms({
  ...
  try {
    await ensureSmsOutboundLogTable();
...
function displayTitleForTemplateKey(key) {
  ...
  if (key === 'reminder_1h') return '1-hour reminder';
