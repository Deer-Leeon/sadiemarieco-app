...
| **SMS** | Yes — admin cancel (below) — **unless staff uncheck “Text the client”** |
| **Notes** | Also cancels on Cal (Cal may email the client). Checkbox is **on by default** (website + iOS). Unchecking still cancels the booking and still fires admin iOS push; only studio SMS is skipped. Client `/manage` cancel is unchanged. |
...
| **Who** | Admin |
| **Status** | `confirmed` |
| **SMS** | Yes — same reschedule text as client — **unless staff uncheck “Text the client”** |
...
| 13 | Admin cancel | admin cancel (off if “Text the client” unchecked) |
| 14 | Admin no-show, no charge | no-show (no fee) |
| 15 | Admin no-show, charge 100% | no-show (with fee) |
| 16 | Admin reschedule | reschedule (off if “Text the client” unchecked) |
...
| Admin god-mode reschedule | `app/api/admin/appointments/[id]/admin-reschedule/route.ts` |
| Silent-SMS skip intent | `app/api/admin/appointments/[id]/client-sms-intent/route.ts` |
